...
class IneuronPredict:
    ...
    def getprediction(self, requestData, csvFilePath, feature, label, modelname):

        #train = pd.read_csv('../data/test.csv')
        train = pd.read_csv(csvFilePath)
        train[feature] = train[feature].str.replace("[^a-zA-Z#]", " ")

        tokenized_tweet = train[feature].apply(lambda x: x.split())  # tokenizing

        bow_vectorizer = CountVectorizer(max_df=0.90, min_df=2, max_features=1000, stop_words='english')
        bow = bow_vectorizer.fit_transform(train[feature])
        train_bow = bow[:, :]
        ...
        model_w2v.train(tokenized_tweet, total_examples=len(train[feature]), epochs=20)
    ...
    def performArithematicOperations (l1, l2, l3):
        multOp = 1
        for val in l1:
            addop = addop + val
            multOp = multOp * val
